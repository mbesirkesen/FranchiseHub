from __future__ import annotations

import re
from dataclasses import dataclass, field
from typing import TYPE_CHECKING, Optional

if TYPE_CHECKING:
    from .agent_vocabulary import AgentVocabulary

from .agent_nlu_helpers import (
    QUERY_STOP_WORDS,
    normalize_agent_text,
    pick_best_free_token,
    strip_city_suffix,
)
from .models import Buyer
from .region_filters import REGION_LABELS

# Frontend agent-query-parser ile uyumlu sektör eşlemeleri
SECTOR_KEYWORDS: list[tuple[tuple[str, ...], str]] = [
    (("fast-food", "fast food", "fastfood", "burger", "hamburger"), "Gıda"),
    (("gıda", "gida", "restoran", "yemek", "bayilik"), "Gıda"),
    (("kahve", "cafe", "kafe", "coffee"), "Kahve"),
    # DB'de sektör "Kafe" yazan markalar için
    (("kafe",), "Kafe"),
    (("güzellik", "guzellik", "kuaför", "kuafor", "güzellik"), "Güzellik"),
    (("perakende", "mağaza", "magaza", "retail"), "Perakende"),
    (("otomotiv", "oto", "araba", "lastik"), "Otomotiv"),
    (("temizlik", "clean", "hijyen", "dry clean"), "Temizlik"),
    (("egitim", "eğitim", "kurs", "okul", "anaokulu"), "Eğitim"),
    (("saglik", "sağlık", "klinik", "eczane"), "Sağlık"),
    (("spor", "fitness", "gym", "spor salonu"), "Spor"),
]

REGION_KEYWORDS: list[tuple[tuple[str, ...], str]] = [
    (("marmara",), "marmara"),
    (("ege",), "ege"),
    (("akdeniz",), "akdeniz"),
    (("iç anadolu", "ic anadolu", "icanadolu", "içanadolu"), "icanadolu"),
    (("karadeniz",), "karadeniz"),
    (("doğu anadolu", "dogu anadolu", "doğu", "dogu"), "dogu"),
    (("güneydoğu", "guneydogu"), "guneydogu"),
]

CITY_KEYWORDS: dict[str, str] = {
    "istanbul": "İstanbul",
    "ankara": "Ankara",
    "izmir": "İzmir",
    "bursa": "Bursa",
    "antalya": "Antalya",
    "adana": "Adana",
    "konya": "Konya",
    "gaziantep": "Gaziantep",
}

COMPARE_HINTS = ("karşılaştır", "karsilastir", "compare")
COMPARE_WITH_OWNED_HINTS = (
    "bende olan",
    "bende ki",
    "bendeki",
    "mevcut bayilik",
    "onayli bayilik",
    "onaylı bayilik",
    "bayiliklerim",
    "bayiliklerimle",
    "sahip oldugum",
    "sahip olduğum",
    "elimdeki",
    "su an bende",
    "şu an bende",
    "aktif bayilik",
    "bayiligim",
    "bayiliğim",
)
COMPARE_WITH_PREVIOUS_HINTS = (
    "digerleriyle",
    "diğerleriyle",
    "onceki liste",
    "önceki liste",
    "listedeki",
    "gosterdigin",
    "gösterdiğin",
    "az once",
    "az önce",
    "yukaridaki",
    "yukarıdaki",
    "bir daha",
    "tekrar",
    "yeniden",
    "yine",
    "ayni karsilastir",
    "aynı karşılaştır",
    "son karsilastir",
    "son karşılaştır",
)
DETAIL_HINTS = ("roi", "getiri", "trend", "şube", "sube", "metrik", "kaç şube", "büyüme")
APPLICATION_HINTS = ("başvuru", "basvuru", "application", "başvurum", "basvurum", "durumda", "başvurularım", "basvurularim")
GENERAL_HINTS = ("franchise nedir", "bayilik nedir", "nasıl çalışır", "nasil calisir")
GREETING_HINTS = (
    "merhaba", "selam", "naber", "gunaydin", "günaydın", "iyi gunler", "iyi günler",
    "nasilsin", "nasılsın", "hosgeldin", "hoşgeldin", "hey", "hello", "hi",
)
HELP_HINTS = (
    "yardim", "yardım", "nasil kullanilir", "nasıl kullanılır", "ne yapabilirsin",
    "ne yaparsin", "ne yaparsın", "komutlar", "ornek soru", "örnek soru",
)
THANKS_HINTS = ("tesekkur", "teşekkür", "sagol", "sağol", "eyvallah", "tamamdir", "tamamdır")
# Kişisel öneri: profil sektörü/bütçesi devreye girebilir
RECOMMEND_HINTS = (
    "onerir", "önerir", "tavsiye", "ne onerirsin", "ne önerirsin", "bana uygun",
    "uygun marka", "öneri", "oneri", "goster bana", "göster bana", "bul bana",
    "bir sey oner", "bir şey öner", "bana gore", "bana göre",
)
# Genel listeleme: profil filtreleri EKLENMEZ, ham liste döner
GENERAL_LIST_HINTS = (
    "ne var", "neler var", "hangi markalar", "hangi marka", "listele",
    "gosterir misin", "gösterir misin", "tum markalar", "tüm markalar",
    "hepsi", "hepsini", "markalar neler",
)
PRODUCT_KEYWORDS = (
    "pizza", "doner", "döner", "burger", "waffle", "tatli", "tatlı", "cicek", "çiçek",
    "komagene", "starbucks", "brew", "sushi", "tost", "cay", "çay", "balik", "balık",
    "tavuk", "pasta", "kuruyemis", "kuruyemiş", "icecek", "içecek", "clean", "studio",
    "auto", "hub", "city", "line", "plus", "max", "zone", "point",
)
BRAND_INFO_HINTS = ("hakkinda", "hakkında", "detay", "bilgi ver", "nedir bu", "tanit", "tanıt")
LIST_HINTS = ("listele", "goster", "göster", "hangi markalar", "neler var", "hepsini", "tumunu", "tümünü")
ROI_HINTS = ("roi", "getiri", "karli", "kârlı", "kar getirisi", "yuksek getiri", "yüksek getiri")
FAVORITES_HINTS = ("favori", "favorilerim", "favorilerime", "favorilerimde")
EXCLUDE_APPLIED_HINTS = (
    "basvurdugum haric",
    "başvurduğum hariç",
    "daha once basvur",
    "daha önce başvur",
    "basvurmadigim",
    "başvurmadığım",
)
TERRITORY_HINTS = ("müsait bölge", "musait bolge", "territory", "bölge var mı", "bolge var mi", "müsaitlik")
GIBBERISH_MIN_ALPHA = 3
# classify_intent ile belirlenen niyetler gibberish ile ezilmemeli
GIBBERISH_EXEMPT_INTENTS = frozenset(
    {
        "application_status",
        "brand_compare",
        "favorites_similar",
        "general",
        "brand_detail",
        "territory_check",
    }
)
REFINE_HINTS = (
    "daha ucuz",
    "daha pahali",
    "daha pahalı",
    "daha az",
    "daha cok",
    "daha çok",
    "daha uygun",
    "bunlar",
    "bunların",
    "bunlarin",
    "aynı",
    "ayni",
    "devam",
    "baska",
    "başka",
    "peki ya",
    "peki",
    "onun yerine",
    "bunun disinda",
    "bunun dışında",
    "goster",
    "göster",
    "olsun",
    "degil",
    "değil",
    "sektor",
    "sektör",
    "yerine",
    "baska sektor",
    "başka sektör",
    "en ucuz",
    "en ucuzu",
    "en pahali",
    "en pahalı",
    "en ucuza",
    "hangisi",
    "hangisini",
    "en iyi",
    "en uygun",
    "kac tane",
    "kaç tane",
    "sadece",
    "filtrele",
    "genislet",
    "genişlet",
    "baska sehir",
    "başka şehir",
    "sonraki",
    "onceki",
    "önceki",
)


@dataclass
class AgentSearchFilters:
    sector: Optional[str] = None
    location: Optional[str] = None
    region: Optional[str] = None
    min_cost: Optional[float] = None
    max_cost: Optional[float] = None
    q: Optional[str] = None
    use_profile_budget: bool = False
    use_profile_sector: bool = False
    use_profile_city: bool = False
    sort: str = "cost_asc"
    exclude_applied: bool = False
    similar_to_favorites: bool = False


@dataclass
class AgentNluResult:
    intent: str
    filters: AgentSearchFilters = field(default_factory=AgentSearchFilters)
    free_text_tokens: list[str] = field(default_factory=list)
    compare_brand_names: list[str] = field(default_factory=list)
    compare_with_owned: bool = False
    compare_with_previous: bool = False
    pick_mode: Optional[str] = None  # cheapest | expensive | best_match | best_roi | ordinal
    pick_index: Optional[int] = None
    db_brand_ids: list[int] = field(default_factory=list)


def _normalize(text: str) -> str:
    return normalize_agent_text(text)


def _parse_money_tl(text: str) -> tuple[Optional[float], Optional[float]]:
    """(min_cost, max_cost) — altı/üstü ifadelerine göre."""
    norm = _normalize(text)
    min_cost: Optional[float] = None
    max_cost: Optional[float] = None

    def _amount_from_match(num_str: str, unit: str) -> float:
        num = float(num_str.replace(",", "."))
        if unit in ("milyon", "m"):
            return num * 1_000_000
        if unit in ("bin", "k"):
            return num * 1_000
        return num

    patterns = [
        r"(\d+(?:[.,]\d+)?)\s*(milyon|m)\b",
        r"(\d+(?:[.,]\d+)?)\s*(bin|k)\b",
        r"(\d+(?:[.,]\d+)?)\s*tl\b",
    ]
    amounts: list[tuple[float, str]] = []
    for pat in patterns:
        for m in re.finditer(pat, norm):
            unit = m.group(2) if m.lastindex and m.lastindex >= 2 else ""
            amounts.append((_amount_from_match(m.group(1), unit), m.group(0)))

    if not amounts and re.search(r"\b(\d{4,12})\b", norm):
        for m in re.finditer(r"\b(\d{4,12})\b", norm):
            amounts.append((float(m.group(1)), m.group(0)))

    # "X ile Y arası" / "X - Y arası" / "X ila Y" → aralık (min, max)
    range_signal = any(
        w in norm for w in ("arasi", "arası", " ile ", " ila ", "araliginda", "aralığında")
    ) or re.search(r"\d\s*[-–]\s*\d", norm)
    if len(amounts) >= 2 and range_signal:
        values = sorted(v for v, _ in amounts)
        return values[0], values[-1]

    for value, fragment in amounts:
        if any(w in fragment for w in ("alti", "altı", "under", "kadar", "max")) or any(
            w in norm for w in ("alti", "altı", "kadar", "under", "max")
        ):
            max_cost = value if max_cost is None else min(max_cost, value)
        elif any(w in norm for w in ("ustu", "üstü", "min", "en az", "uzeri", "üzeri")):
            min_cost = value if min_cost is None else max(min_cost, value)
        else:
            max_cost = value if max_cost is None else min(max_cost, value)

    # "bütçem" geçse de açık bir tutar yazıldıysa onu kullan; yoksa profil
    # bütçesini caller işler.
    if min_cost is None and max_cost is None and re.search(
        r"butceme|bütçeme|butcem|bütçem", norm
    ):
        return None, None

    return min_cost, max_cost


def _query_has_explicit_budget(
    norm: str,
    min_parsed: Optional[float],
    max_parsed: Optional[float],
) -> bool:
    """Sorguda yazılı tutar varsa profil sektörü gibi daraltıcı varsayılanları zayıflat."""
    if min_parsed is not None or max_parsed is not None:
        return True
    return bool(
        re.search(r"\d{5,}", norm)
        or re.search(r"\d+\s*(milyon|bin|tl)\b", norm)
    )


def _detect_sector(norm: str, vocabulary: Optional[AgentVocabulary] = None) -> Optional[str]:
    if vocabulary:
        matched = vocabulary.match_sector(norm)
        if matched:
            return matched
    for keywords, sector in SECTOR_KEYWORDS:
        if any(kw in norm for kw in keywords):
            # Sözlük varsa, keyword eşlemesi DB'de gerçekten olmayan bir sektöre
            # işaret ediyorsa (ör. "Spor") uygulama — aksi halde 0 sonuç + alakasız öneri.
            if vocabulary and vocabulary.sectors and sector not in vocabulary.sectors:
                return None
            return sector
    return None


def _detect_region(norm: str) -> Optional[str]:
    for keywords, key in REGION_KEYWORDS:
        if any(kw in norm for kw in keywords):
            return key
    return None


def _detect_city(
    norm: str, vocabulary: Optional[AgentVocabulary] = None
) -> Optional[str]:
    if vocabulary:
        matched = vocabulary.match_location(norm)
        if matched:
            return matched
    city_key = strip_city_suffix(norm)
    if city_key and city_key in CITY_KEYWORDS:
        return CITY_KEYWORDS[city_key]
    for key, label in CITY_KEYWORDS.items():
        if key in norm:
            return label
    return None


def _is_compare_with_owned_franchises(norm: str) -> bool:
    if any(h in norm for h in COMPARE_WITH_OWNED_HINTS):
        return True
    return "bayilik" in norm and any(w in norm for w in ("bende", "benim", "sahibiyim"))


def _is_compare_with_previous_list(norm: str) -> bool:
    return any(h in norm for h in COMPARE_WITH_PREVIOUS_HINTS)


def _is_compare_intent(norm: str, raw: str) -> bool:
    # Para/bütçe aralığı ("500 bin ile 1 milyon arası") karşılaştırma değildir.
    money_present = bool(re.search(r"\d+\s*(bin|milyon|tl|k|m)\b", norm)) or any(
        w in norm for w in ("arasi", "arası", "butce", "bütçe", "alti", "altı")
    )
    if any(h in norm for h in COMPARE_HINTS) or " vs " in norm:
        # "karşılaştır" açıkça varsa compare; ama yalnızca para varsa değil
        if not money_present or len(extract_compare_brand_names(raw)) >= 2:
            return True
    # «Komagene ile Brew Max» — yalnızca karşılaştırma fiili veya iki marka adı
    if (
        re.search(r"\s+ile\s+", norm)
        and not money_present
        and not re.search(
            r"ne yapabilirim|ne yapabilir|nasil|nasıl|yapmak|istiyorum", norm
        )
    ):
        names = extract_compare_brand_names(raw)
        if len(names) >= 2:
            return True
    return False


def _is_gibberish(norm: str) -> bool:
    if any(h in norm for h in APPLICATION_HINTS + COMPARE_HINTS + FAVORITES_HINTS + GENERAL_HINTS):
        return False
    if any(
        h in norm
        for h in GREETING_HINTS + HELP_HINTS + THANKS_HINTS + RECOMMEND_HINTS
        + PRODUCT_KEYWORDS + LIST_HINTS + ROI_HINTS
    ):
        return False
    if any(h in norm for h in REFINE_HINTS):
        return False
    letters = re.sub(r"[^a-z]", "", norm)
    if len(letters) < GIBBERISH_MIN_ALPHA:
        return True
    known = set()
    for kws, _ in SECTOR_KEYWORDS + REGION_KEYWORDS:
        known.update(kws)
    known.update(CITY_KEYWORDS.keys())
    known.update(
        "butceme butce bütçe marmara gıda kahve franchise bayilik marka roi alti altı".split()
    )
    if any(k in norm for k in known):
        return False
    if re.search(r"\d+\s*(bin|k|milyon|tl)|butce|bütç", norm):
        return False
    if len(letters) <= 14:
        return True
    return not any(w in norm for w in ("franchise", "bayilik", "marka"))


_COMPARE_STOP_TOKENS = frozenset(
    {
        "marka", "markalar", "franchise", "bayilik", "hangisi", "daha", "iyi",
        "misin", "musun", "musunuz", "misiniz", "misin", "lutfen", "lütfen",
        "icin", "için", "bana", "benim", "ile", "ver", "eder",
        "bin", "milyon", "tl", "arasi", "arası", "alti", "altı", "ustu", "üstü",
        "butce", "bütçe", "butcem", "bütçem",
    }
)


def extract_compare_brand_names(query: str) -> list[str]:
    norm = _normalize(query)
    # Tüm "karşılaştır..." varyantlarını (karsilastirir, karsilastirabilir) temizle
    norm = re.sub(r"kar[sş][iı]la[sş]t[iı]r\w*", " ", norm)
    for token in ("compare", "markalari", "markaları", "kiyasla", "kıyasla"):
        norm = norm.replace(token, " ")
    parts = re.split(r"\s+(?:ile|ve|vs|veya)\s+", norm)
    names: list[str] = []
    for part in parts:
        cleaned = re.sub(r"[^a-z0-9\s]", " ", part)
        # Soru ekleri / genel kelimeleri / salt sayıları at
        tokens = [
            t for t in cleaned.split()
            if t not in _COMPARE_STOP_TOKENS and not t.isdigit()
        ]
        cleaned = " ".join(tokens).strip()
        if len(cleaned) < 3:
            continue
        title = " ".join(w.capitalize() for w in cleaned.split())
        if title and title not in names:
            names.append(title)
    return names[:4]


def classify_intent(query: str, brand_id: Optional[int]) -> str:
    norm = _normalize(query)
    if brand_id is not None and any(h in norm for h in TERRITORY_HINTS):
        return "territory_check"
    if brand_id is not None and any(h in norm for h in DETAIL_HINTS):
        return "brand_detail"
    if any(h in norm for h in APPLICATION_HINTS):
        return "application_status"
    if any(h in norm for h in FAVORITES_HINTS) and any(
        w in norm for w in ("benzer", "oner", "öner", "gibi", "similar")
    ):
        return "favorites_similar"
    if _is_compare_intent(norm, query):
        return "brand_compare"
    if any(h in norm for h in GENERAL_HINTS):
        return "general"
    if any(h in norm for h in THANKS_HINTS):
        return "general"
    if any(norm == g or norm.startswith(g + " ") for g in GREETING_HINTS + HELP_HINTS):
        return "general"
    if any(h in norm for h in RECOMMEND_HINTS + LIST_HINTS):
        return "brand_search"
    if any(h in norm for h in ROI_HINTS):
        return "brand_search"
    if any(p in norm for p in PRODUCT_KEYWORDS):
        return "brand_search"
    if any(h in norm for h in BRAND_INFO_HINTS):
        return "brand_search"
    if _is_gibberish(norm):
        return "no_match"
    has_money = bool(
        re.search(r"\d+\s*(bin|k|milyon|m\b|tl\b)", norm)
        or re.search(r"\b\d{4,12}\b", norm)
        or "butce" in norm
        or "bütç" in norm
    )
    if (
        _detect_sector(norm)
        or _detect_region(norm)
        or _detect_city(norm)
        or has_money
        or "franchise" in norm
        or "bayilik" in norm
        or "marka" in norm
    ):
        return "brand_search"
    return "no_match"


def _filters_from_applied(data: Optional[dict]) -> AgentSearchFilters:
    if not data:
        return AgentSearchFilters()
    f = AgentSearchFilters()
    if data.get("sector"):
        f.sector = str(data["sector"])
    if data.get("location"):
        f.location = str(data["location"])
    if data.get("region"):
        f.region = str(data["region"])
    if data.get("min_cost") is not None:
        f.min_cost = float(data["min_cost"])
    if data.get("max_cost") is not None:
        f.max_cost = float(data["max_cost"])
    if data.get("q"):
        f.q = str(data["q"])
    if data.get("use_profile_budget"):
        f.use_profile_budget = True
    if data.get("use_profile_sector"):
        f.use_profile_sector = True
    if data.get("use_profile_city"):
        f.use_profile_city = True
    if data.get("exclude_applied"):
        f.exclude_applied = True
    if data.get("sort"):
        f.sort = str(data["sort"])
    return f


def _detect_pick_mode(norm: str) -> Optional[str]:
    """Önceki listeden seçim: en ucuz hangisi, en iyisi vb."""
    if re.search(r"en ucuz|en ucuzu|en ucuza|en dusuk|en düşük", norm):
        return "cheapest"
    if re.search(r"hangisi", norm) and re.search(r"ucuz", norm):
        return "cheapest"
    if re.search(r"en pahali|en pahalı|en yuksek|en yüksek|en pahalisi", norm):
        return "expensive"
    if re.search(r"hangisi", norm) and re.search(r"pahali|pahalı", norm):
        return "expensive"
    if re.search(r"en iyi|en uygun|hangisi iyi|hangisi daha iyi|en karli|en kârlı", norm):
        return "best_match"
    if re.search(r"en yuksek roi|en yüksek roi|roi en yuksek", norm):
        return "best_roi"
    if norm.strip() in ("hangisi", "hangisini", "en iyisi", "en ucuzu"):
        return "cheapest" if "ucuz" in norm else "best_match"
    if re.search(r"^hangisi\b", norm):
        return "cheapest"
    if re.search(r"^(birinci|ikinci|ucuncu|üçüncu|son|ilk)\b", norm):
        return "ordinal"
    if re.search(r"\b\d+\.\s*(marka|sira|sıra)\b", norm):
        return "ordinal"
    return None


def _detect_ordinal_index(norm: str) -> int:
    words = {
        "birinci": 0, "ilk": 0, "1": 0,
        "ikinci": 1, "2": 1,
        "ucuncu": 2, "üçüncü": 2, "3": 2,
        "dorduncu": 3, "dördüncü": 3, "4": 3,
        "son": -1, "sonuncu": -1,
    }
    for key, idx in words.items():
        if re.search(rf"\b{key}\b", norm):
            return idx
    m = re.search(r"\b(\d+)\.\s*(?:marka|sira|sıra)?", norm)
    if m:
        return max(0, int(m.group(1)) - 1)
    return 0


def _is_refine_follow_up(norm: str) -> bool:
    return any(h in norm for h in REFINE_HINTS) or _detect_pick_mode(norm) is not None


def _is_context_follow_up(norm: str, previous_search: Optional[dict]) -> bool:
    """Önceki aramaya bağlı kısa takip sorusu (sektör/şehir değişimi dahil)."""
    if not previous_search:
        return False
    if _is_refine_follow_up(norm):
        return True
    if len(norm.split()) > 8:
        return False
    return bool(_detect_sector(norm) or _detect_city(norm) or _detect_region(norm))


def _merge_query_overrides(filters: AgentSearchFilters, norm: str) -> None:
    sector = _detect_sector(norm)
    if sector:
        filters.sector = sector
        filters.use_profile_sector = False
    city = _detect_city(norm)
    if city:
        filters.location = city
        filters.use_profile_city = False
    region = _detect_region(norm)
    if region:
        filters.region = region
    min_parsed, max_parsed = _parse_money_tl(norm)
    if min_parsed is not None:
        filters.min_cost = min_parsed
    if max_parsed is not None:
        filters.max_cost = max_parsed
        filters.use_profile_budget = False


def _apply_conversational_filters(filters: AgentSearchFilters, norm: str, buyer: Buyer) -> None:
    if re.search(r"\bucuz\b", norm):
        filters.sort = "cost_asc"
        if filters.q in ("ucuz", "ucuzlar", "ucuza"):
            filters.q = None
    elif re.search(r"\bpahali|pahalı|premium|luks|lüks\b", norm):
        filters.sort = "cost_desc"
    if re.search(r"\biyi\b|en iyi|roi|getiri|yuksek getiri|yüksek getiri", norm):
        filters.sort = "roi_desc"
    if re.search(
        r"butceme|bütçeme|butcem|bütçem|butceme gore|bütçeme göre|oner|öner|tavsiye|bana uygun|uygun marka",
        norm,
    ):
        filters.use_profile_budget = True
        filters.max_cost = float(buyer.investment_budget)
    for product in PRODUCT_KEYWORDS:
        if product in norm and not filters.q:
            filters.q = product
            break
    if filters.location and filters.q and filters.q in CITY_KEYWORDS:
        filters.q = None


def extract_brand_name_tokens(norm: str) -> list[str]:
    stop = {
        "hakkinda", "hakkında", "bilgi", "detay", "nedir", "marka", "franchise",
        "bayilik", "icin", "için", "ver", "lutfen", "lütfen",
    }
    tokens = [t for t in re.split(r"\s+", norm) if len(t) >= 3 and t not in stop]
    return [" ".join(w.capitalize() for w in t.split()) for t in tokens[:4]]


def _apply_refine_to_filters(filters: AgentSearchFilters, norm: str, buyer: Buyer) -> None:
    if re.search(r"daha ucuz|daha az|daha uygun", norm):
        filters.sort = "cost_asc"
        if filters.max_cost is not None:
            filters.max_cost = filters.max_cost * 0.85
        elif filters.use_profile_budget:
            filters.max_cost = float(buyer.investment_budget) * 0.85
    elif re.search(r"daha pahali|daha pahalı|daha cok|daha çok", norm):
        filters.sort = "cost_desc"
        if filters.max_cost is not None:
            filters.min_cost = filters.max_cost * 0.9
            filters.max_cost = None
    if re.search(r"butceme|bütçeme|butcem|bütçem", norm):
        filters.use_profile_budget = True
        filters.max_cost = float(buyer.investment_budget)


def parse_agent_query(
    query: str,
    buyer: Buyer,
    *,
    previous_search: Optional[dict] = None,
    vocabulary: Optional[AgentVocabulary] = None,
) -> AgentNluResult:
    raw = query.strip()
    norm = _normalize(raw)
    intent = classify_intent(raw, None)

    filters = AgentSearchFilters()
    filters.sector = _detect_sector(norm, vocabulary)
    filters.region = _detect_region(norm)
    filters.location = _detect_city(norm, vocabulary)
    db_brand_ids: list[int] = []
    if vocabulary:
        db_brand_ids = vocabulary.match_brand_ids(norm)

    min_parsed, max_parsed = _parse_money_tl(raw)
    if re.search(r"butceme|bütçeme|butcem|bütçem", norm):
        filters.use_profile_budget = True
        filters.max_cost = float(buyer.investment_budget)
    else:
        filters.min_cost = min_parsed
        filters.max_cost = max_parsed

    tokens = [t for t in re.split(r"\s+", norm) if len(t) >= 3]
    free = [t for t in tokens if t not in QUERY_STOP_WORDS and not t.isdigit()]
    if free and not filters.q:
        token = pick_best_free_token(free)
        if token:
            filters.q = token
        elif vocabulary:
            vocab_tokens = vocabulary.extract_search_tokens(norm)
            if vocab_tokens:
                filters.q = vocab_tokens[0]
    if filters.location and filters.q:
        filters.q = None

    compare_names: list[str] = []
    compare_with_owned = False
    compare_with_previous = False
    if intent == "brand_compare":
        compare_names = extract_compare_brand_names(raw)
        compare_with_owned = _is_compare_with_owned_franchises(norm)
        compare_with_previous = _is_compare_with_previous_list(norm)

    if any(h in norm for h in EXCLUDE_APPLIED_HINTS):
        filters.exclude_applied = True

    if _is_gibberish(norm) and intent not in GIBBERISH_EXEMPT_INTENTS:
        intent = "no_match"

    if intent == "no_match" and vocabulary and (
        db_brand_ids or filters.sector or filters.location
    ):
        intent = "brand_search"

    pick_mode = _detect_pick_mode(norm)
    pick_index: Optional[int] = None
    if pick_mode and previous_search:
        prev_ids = previous_search.get("related_brand_ids") or []
        if prev_ids:
            intent = "brand_pick"
            if pick_mode == "ordinal":
                pick_index = _detect_ordinal_index(norm)
            prev_filters = previous_search.get("filters_applied")
            if isinstance(prev_filters, dict) and prev_filters:
                filters = _filters_from_applied(prev_filters)

    if intent == "no_match" and _is_context_follow_up(norm, previous_search):
        prev_filters = previous_search.get("filters_applied") if previous_search else None
        prev_ids = (previous_search or {}).get("related_brand_ids") or []
        if pick_mode and prev_ids:
            intent = "brand_pick"
            if pick_mode == "ordinal":
                pick_index = _detect_ordinal_index(norm)
            if isinstance(prev_filters, dict) and prev_filters:
                filters = _filters_from_applied(prev_filters)
        elif isinstance(prev_filters, dict):
            intent = "brand_search"
            filters = _filters_from_applied(prev_filters) if prev_filters else AgentSearchFilters()
            _apply_refine_to_filters(filters, norm, buyer)
            _merge_query_overrides(filters, norm)

    # Profil varsayılanları — yalnızca anlamlı brand_search sorgularında
    if intent == "brand_search":
        _apply_conversational_filters(filters, norm, buyer)
        # Kullanıcı şehir/bölge belirttiyse veya "hangi markalar var" gibi genel
        # listeleme yaptıysa profil sektör/bütçesini ZORLA ekleme — aksi halde
        # sonuçlar gereksiz daralır.
        user_scoped = filters.location is not None or filters.region is not None
        general_list = any(h in norm for h in GENERAL_LIST_HINTS)
        # "başka/farklı sektör öner" → kullanıcı çeşitlilik istiyor; profil sektörünü
        # (tek sektör) zorlamak yanlış olur, sektörsüz genel öneri ver.
        wants_other = (
            any(w in norm for w in ("baska", "başka", "farkli", "farklı", "degisik", "değişik", "diger", "diğer"))
            and "sekt" in norm
        )
        allow_profile_defaults = not user_scoped and not general_list and not wants_other
        explicit_budget = _query_has_explicit_budget(norm, min_parsed, max_parsed)
        if filters.max_cost is None and filters.use_profile_budget:
            filters.max_cost = float(buyer.investment_budget)
        if (
            allow_profile_defaults
            and not explicit_budget
            and filters.sector is None
            and (
                filters.use_profile_budget
                or re.search(
                    r"butceme|bütçeme|öner|oner|tavsiye|bana uygun|ne onerirsin|ne önerirsin",
                    norm,
                )
            )
        ):
            if buyer.preferred_sector:
                filters.use_profile_sector = True
                filters.sector = buyer.preferred_sector
        if filters.location is None and filters.region is None and buyer.city:
            if re.search(r"yakın|yakin|yakınımda|sehrim|şehrim|is kurmak|iş kurmak", norm):
                filters.use_profile_city = True
                filters.location = buyer.city
        if (
            allow_profile_defaults
            and filters.sector is None
            and filters.max_cost is None
            and not filters.q
            and any(h in norm for h in RECOMMEND_HINTS)
        ):
            filters.use_profile_budget = True
            filters.max_cost = float(buyer.investment_budget)
            if buyer.preferred_sector and not explicit_budget:
                filters.use_profile_sector = True
                filters.sector = buyer.preferred_sector
        if filters.max_cost is not None and re.search(r"ne yapabilirim|ne yapabilir", norm):
            if filters.q in ("ile", "ve", "ne"):
                filters.q = None
            if buyer.preferred_sector and filters.sector is None and not explicit_budget:
                filters.sector = buyer.preferred_sector
                filters.use_profile_sector = True
        if (
            vocabulary
            and not filters.sector
            and not filters.location
            and not filters.q
            and not filters.max_cost
            and not db_brand_ids
            and len(norm.split()) >= 2
        ):
            filters.q = raw.strip()[:100]

    return AgentNluResult(
        intent=intent,
        filters=filters,
        free_text_tokens=free,
        compare_brand_names=compare_names,
        compare_with_owned=compare_with_owned,
        compare_with_previous=compare_with_previous,
        pick_mode=pick_mode if intent == "brand_pick" else None,
        pick_index=pick_index if intent == "brand_pick" else None,
        db_brand_ids=db_brand_ids,
    )


def build_profile_recommendation_filters(buyer: Buyer) -> AgentSearchFilters:
    """Son çare: profil bütçe + sektör ile genel öneri."""
    f = AgentSearchFilters()
    f.use_profile_budget = True
    f.max_cost = float(buyer.investment_budget)
    if buyer.preferred_sector:
        f.sector = buyer.preferred_sector
        f.use_profile_sector = True
    if buyer.city:
        f.location = buyer.city
        f.use_profile_city = True
    return f


def filters_applied_dict(filters: AgentSearchFilters) -> dict[str, object]:
    out: dict[str, object] = {"sort": filters.sort}
    if filters.sector:
        out["sector"] = filters.sector
    if filters.location:
        out["location"] = filters.location
    if filters.region:
        out["region"] = filters.region
    if filters.min_cost is not None:
        out["min_cost"] = filters.min_cost
    if filters.max_cost is not None:
        out["max_cost"] = filters.max_cost
    if filters.q:
        out["q"] = filters.q
    if filters.use_profile_budget:
        out["use_profile_budget"] = True
    if filters.use_profile_sector:
        out["use_profile_sector"] = True
    if filters.exclude_applied:
        out["exclude_applied"] = True
    if filters.similar_to_favorites:
        out["similar_to_favorites"] = True
    return out


def filters_human_label(filters: AgentSearchFilters, profile_budget: float) -> str:
    parts: list[str] = []
    if filters.sector:
        parts.append(filters.sector)
    if filters.region:
        parts.append(REGION_LABELS.get(filters.region, filters.region))
    if filters.location:
        parts.append(filters.location)
    if filters.max_cost is not None:
        if filters.use_profile_budget:
            parts.append(f"Profilinizdeki {profile_budget:,.0f} TL bütçe")
        else:
            parts.append(f"{filters.max_cost:,.0f} TL altı")
    if filters.min_cost is not None:
        parts.append(f"{filters.min_cost:,.0f} TL üstü")
    if filters.q:
        parts.append(f"«{filters.q}»")
    return " · ".join(parts) if parts else "genel arama"
